...
///
/// The first two lines rather than only the first, because a generated figure
/// cannot put this in a comment: an XML comment may not contain `--`, and the
/// commands all have `--tiers` in them. A figure carries it in an SVG
/// `<metadata>` element instead, which has to sit inside the root element and
/// so lands on line two. See `crate::plot::write_figure`.
pub(crate) fn read_provenance(root: &Path, relative: &str) -> Option<(String, String)> {
    let text = fs::read_to_string(root.join(relative)).ok()?;
    let rest = text
        .lines()
        .take(2)
        .find_map(|line| line.split_once(PROVENANCE))?
        .1;
...
///
/// `figures` is markdown that inlines whatever SVGs [`crate::plot`] drew from
/// these same runs, and it goes above the tables on purpose: the picture is
/// what a reader checks first, and the table is what they check it against.
